...
export default function WordToPdfPage() {
  ...
  const handleConvert = async () => {
    ...
    try {
      const response = await fetch("http://localhost:8000/convert/word-to-pdf", {
        method: "POST",
        body: formData,
      });
    ...
    <main className="min-h-screen bg-[#020202] text-white overflow-hidden">
      ...
      <div className="relative pt-32 pb-20 px-6 max-w-4xl mx-auto z-10">
        ...
        >
          ...
          <h1 className="text-5xl font-bold mb-4 tracking-tight">Word to <span className="text-blue-400">PDF</span></h1>
          <p className="text-gray-400 text-lg max-w-xl mx-auto">
            Convert your Microsoft Word documents into high-quality PDFs 
            while perfectly preserving every detail and layout.
          </p>
        </motion.div>
        ...
        <div className="bg-[#0A0A0A]/80 backdrop-blur-2xl border border-white/5 rounded-3xl p-8 shadow-2xl">
          {status === "idle" && (
            <div className="space-y-8">
              {!file ? (
                <div 
                  className="border-2 border-dashed border-white/10 rounded-2xl p-16 text-center hover:border-blue-500/40 transition-all cursor-pointer group relative overflow-hidden"
                ...
                >
                  ...
                  <div className="relative z-10">
                    ...
                    <p className="text-2xl font-medium text-white mb-2">Select Word Document</p>
                    <p className="text-gray-500">Supports .DOCX and .DOC files</p>
                  </div>
                </div>
              ) : (
                <motion.div 
              ...
              <button
                ...
                className={`w-full py-5 rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-lg ${
                  file 
                    ? "bg-gradient-to-r from-blue-600 to-blue-400 hover:opacity-90 shadow-blue-500/20" 
          ...
          {(status === "uploading" || status === "processing") && (
            <div className="py-20 text-center">
              ...
              <h3 className="text-2xl font-bold mb-4">
                {status === "uploading" ? "Uploading Document..." : "Generating PDF..."}
              </h3>
